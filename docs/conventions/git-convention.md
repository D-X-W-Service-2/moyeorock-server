...
새 라이브러리·통신 방식·설계 패턴 등 **기존 컨벤션(`docs/conventions/*`)에 없던 기술적 선택**을 할 때마다 무조건 기록한다.

- **`docs/plans`**: 무엇을 썼는지, 왜 그걸 골랐는지, 후보로 검토한 대안이 뭐였는지 적는다
- **PR 본문**: `.github/pull_request_template.md`의 "🛠️ 기술 선택 및 대안" 절에 위 내용을 요약해서 옮긴다. 새 기술 선택이 없는 PR은 "해당 없음"이라고 적는다

이미 컨벤션에 정해진 방식을 그대로 따르는 경우(예: `record`로 DTO 선언, `@Transactional` 위치 등)는 대상이 아니다. **팀이 처음 결정하는 지점**만 기록 대상이다.
...
- **CI 필수 체크**: `gradle check`가 통과해야 Merge할 수 있다.
